...
class Azure:
    ...
    def _initialize_app_config_client(self):
        ...
        if not connection_string:
            self.logger.log(logger.LogLevel.ERROR, "A variável de ambiente 'AppConfigConnectionString' não está definida.")
    ...
    def _initialize_key_vault_client(self):
        ...
        if key_vault_url is None:
            self.logger.log(logger.LogLevel.ERROR, "A URL do Azure Key Vault não foi encontrada na configuração.")
        ...
        if not key_vault_url.startswith("https://") or ".vault.azure.net" not in key_vault_url:
            self.logger.log(logger.LogLevel.ERROR, "URL do Azure Key Vault fornecida está incorreta")
    ...
    def get_app_config_setting(self, key, label="Badge"):
        try:
            ...
            self.logger.log(logger.LogLevel.ERROR, f"Erro ao obter a configuração para a chave '{key}': {str(e)}\nStack Trace:\n{stack_trace}")
            return None
    ...
    def get_key_vault_secret(self, secret_name):
        try:
            ...
            stack_trace = traceback.format_exc()
            self.logger.log(logger.LogLevel.ERROR, f"Erro ao obter o segredo '{secret_name}' do Azure Key Vault: {str(e)}\nStack Trace:\n{stack_trace}")
            return None
