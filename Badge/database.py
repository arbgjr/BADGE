...
class Database:
    def __init__(self):
        ...
        self.logger.log(self.LogLevel.DEBUG, f"[database] Obter dados de conexão com o banco.")
    ...
    def connect(self):
        try:
            self.logger.log(self.LogLevel.DEBUG, f"[database] Conectando com o banco.")
            return pyodbc.connect(self.conn_str)
        except pyodbc.Error as e:
            stack_trace = traceback.format_exc()
            self.logger.log(self.LogLevel.ERROR, f"Erro de conexão com o banco de dados: {e}\nStack Trace:\n{stack_trace}")
            raise
    ...
    def get_badge_template(self, badge_id):
        try:
            ...
            stack_trace = traceback.format_exc()
            self.logger.log(self.LogLevel.ERROR, f"Erro ao obter template do badge: {e}\nStack Trace:\n{stack_trace}")
            return None
    ...
    def get_badge_image(self, badge_guid):
        try:
            ...
            stack_trace = traceback.format_exc()
            self.logger.log(self.LogLevel.ERROR, f"Erro ao obter imagem do badge: {e}\nStack Trace:\n{stack_trace}")
            return None
    ...
    def validate_badge(self, badge_guid, owner_name, issuer_name):
        try:
            ...
            stack_trace = traceback.format_exc()
            self.logger.log(self.LogLevel.ERROR, f"Erro ao validar badge: {e}\nStack Trace:\n{stack_trace}")
            return None
    ...
    def get_user_badges(self, user_id):
        try:
            ...
            stack_trace = traceback.format_exc()
            self.logger.log(self.LogLevel.ERROR, f"Erro ao obter badges do usuário: {e}\nStack Trace:\n{stack_trace}")
            return None
    ...
    def get_badge_holders(self, badge_name):
        try:
            ...
            stack_trace = traceback.format_exc()
            self.logger.log(self.LogLevel.ERROR, f"Erro ao obter detentores do badge: {e}\nStack Trace:\n{stack_trace}")
            return None
    ...
    def get_badge_info_for_post(self, badge_guid):
        try:
            ...
            stack_trace = traceback.format_exc()
            self.logger.log(self.LogLevel.ERROR, f"Erro ao obter informações do badge para postagem: {e}\nStack Trace:\n{stack_trace}")
            return None
    ...
    def insert_badge(self, badge_guid, badge_hash, owner_name, issuer_name, signed_hash, badge_base64):
        try:
            with self.connect() as conn:
                cursor = conn.cursor()
                self.logger.log(self.LogLevel.DEBUG, f"[database] Inserindo dados no banco.")
                cursor.execute(
                ...
                self.logger.log(self.LogLevel.DEBUG, f"[database] Comitando dados .")
                conn.commit()
            return True
        except Exception as e:
            stack_trace = traceback.format_exc()
            self.logger.log(self.LogLevel.ERROR, f"Erro ao inserir badge no banco de dados: {e}\nStack Trace:\n{stack_trace}")
            return False
