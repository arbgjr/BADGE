...
def load_image_from_base64(base64_img):
    try:
        ...
        if not isinstance(base64_img, str):
            logger.log(logger.LogLevel.ERROR, "Dados de entrada não são uma string base64 válida.")
            return None
        ...
        logger.log(logger.LogLevel.ERROR, "Erro na decodificação dos dados base64.")
    except IOError:
        # Erro específico para problemas relacionados à I/O ao abrir a imagem
        logger.log(logger.LogLevel.ERROR, "Não foi possível abrir a imagem a partir dos dados base64.")
    except Exception as e:
        stack_trace = traceback.format_exc()
        # Captura outros tipos de exceções
        logger.log(logger.LogLevel.ERROR, f"Erro ao carregar imagem de base64: {str(e)}\nStack Trace:\n{stack_trace}")
    return None
...
def add_text_to_badge(badge_template, owner_name, issuer_name):
    try:
        ...
        if font is None:
            logger.log(logger.LogLevel.ERROR, "Falha ao carregar a fonte Rubik.")
            return None
        ...
        stack_trace = traceback.format_exc()
        logger.log(logger.LogLevel.ERROR, f"Erro ao adicionar texto ao badge: {str(e)}\nStack Trace:\n{stack_trace}")
        return None

def create_qr_code(data, base_url, box_size=10, border=5):
    if not data or not base_url:
        logger.log(logger.LogLevel.ERROR, "Dados ou URL base não fornecidos para o QR Code.")
        return None
    ...
    try:
        ...
        stack_trace = traceback.format_exc()
        logger.log(logger.LogLevel.ERROR, f"Erro ao criar QR Code: {str(e)}\nStack Trace:\n{stack_trace}")
        return None
...
def process_badge_image(badge_template, issuer_name):
    ...
    try:
        ...
        stack_trace = traceback.format_exc()
        logger.log(logger.LogLevel.ERROR, f"Erro ao processar a imagem do badge: {str(e)}\nStack Trace:\n{stack_trace}")
        return None
...
def load_font(font_path, size):
    try:
        ...
        logger.log(logger.LogLevel.ERROR, f"Não foi possível carregar a fonte: {font_path}\nStack Trace:\n{stack_trace}")
    except Exception as e:
        stack_trace = traceback.format_exc()
        # Captura outros tipos de exceções
        logger.log(logger.LogLevel.ERROR, f"Erro ao carregar a fonte ({font_path}): {str(e)}\nStack Trace:\n{stack_trace}")
    return None
...
def generate_image_hash(image):
    try:
        ...
        if not isinstance(image, Image.Image):
            logger.log(logger.LogLevel.ERROR, "O objeto fornecido não é uma imagem válida.")
            return None
        ...
        stack_trace = traceback.format_exc()
        logger.log(logger.LogLevel.ERROR, f"Erro ao gerar o hash da imagem: {str(e)}\nStack Trace:\n{stack_trace}")
        return None
...
def insert_exif(image, exif_data):
    try:
        ...
        if not isinstance(exif_data, dict):
            logger.log(logger.LogLevel.ERROR, "Os dados EXIF fornecidos não estão no formato de dicionário.")
            return None
        ...
        stack_trace = traceback.format_exc()
        logger.log(logger.LogLevel.ERROR, f"Erro ao inserir dados EXIF na imagem: {str(e)}\nStack Trace:\n{stack_trace}")
        return None
