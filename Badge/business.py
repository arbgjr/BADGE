...
def get_configs():
    try:
        logger.log_debug(f"[business] Endpoint para recuperar configurações.")
        data = {}
        ...
        logger.log_error(f"Erro ao recuperar informações: {str(e)}\nStack Trace:\n{stack_trace}")
...
def generate_badge(data):
    try:
        ...
        logger.log_debug(f"[business] Endpoint para emitir um novo badge.")
        if 'owner_name' not in data or 'issuer_name' not in data:
            logger.log_error("Dados de entrada faltando: 'owner_name' ou 'issuer_name'")
            return {"error": "Dados de entrada inválidos"}, 400

        owner_name = data['owner_name']
        issuer_name = data['issuer_name']

        logger.log_debug(f"Gerando badge para {owner_name} emitido por {issuer_name}")

        db = Database()

        # Carregar template de imagem
        logger.log_debug(f"[business] Recuperando template do Badge em base64.")
        template_id = int(azure_client.get_app_config_setting('BadgeTemplateBase64'))
        logger.log_debug(f"[business] Template do Badge em base64: {template_id}.")
        if not template_id:
            logger.log_error("Falha ao carregar id do template do badge.")
            return {"error": "Falha ao carregar id do template do badge"}, 500
        
        badge_template_base64  = db.get_badge_template(template_id)
        logger.log_debug(f"[business] Template do Badge em base64: {badge_template_base64}.")  
        if not badge_template_base64:
            logger.log_error("Template de badge não encontrado.")
            return {"error": "Template de badge não encontrado"}, 500

        logger.log_debug(f"[business] Carregar template de imagem.")
        badge_template = helpers.load_image_from_base64(badge_template_base64)
        logger.log_debug(f"[business] Template do Badge: {badge_template}.")
        if not badge_template:
            logger.log_error("Falha ao carregar template de badge.")
            return {"error": "Falha ao carregar template de badge"}, 500

        logger.log_debug(f"[business] Carregar URL de verificação do Badge.")
        base_url = azure_client.get_app_config_setting('BadgeVerificationUrl')
        logger.log_debug(f"[business] URL de verificação do Badge: {base_url}.")
        if not base_url:
            logger.log_error("Falha ao carregar a URL de verificação do badge.")
            return {"error": "Falha ao carregar url de verificação do badge"}, 500
        
        if not helpers.validar_url_https(base_url):
            logger.log_error("URL de verificação do badge inválida.")
            return {"error": "URL de verificação do badge inválida."}, 500
 
        logger.log_debug(f"[business] Gerando GUID do Badge.")
        badge_guid = helpers.gera_guid_badge() 
        logger.log_debug(f"[business] Gerando dados de verificação do Badge: {badge_guid}.")
        concatenated_data = f"{badge_guid}|{owner_name}|{issuer_name}"
        encrypted_data = helpers.encrypt_data(concatenated_data)

        logger.log_debug(f"[business] Adicionando texto ao Badge.")
        badge_template = helpers.add_text_to_badge(badge_template, owner_name, issuer_name)
        if badge_template is None:
            logger.log_error("Falha ao editar badge. ")
            return {"error": "Falha ao editar badge."}, 500 

        logger.log_debug(f"[business] Gerando QRCode do Badge.")
        qr_code_img = helpers.create_qr_code(encrypted_data, base_url, box_size=10, border=4)
        if qr_code_img is None:
            logger.log_error("Falha ao gerar QR Code. ")
            return {"error": "Falha ao editar badge."}, 500 

        logger.log_debug(f"[business] Inerindo Qrcode no Badge.")
        badge_template.paste(qr_code_img, (10, 50))

        logger.log_debug(f"[business] Inserindo dados EXIF do Badge.")
        result = helpers.process_badge_image(badge_template, issuer_name)
        if result is not None:
            badge_hash, badge_base64, signed_hash = result
        else:
            logger.log_error("Falha ao editar exif do badge. ")
            return {"error": "Falha ao editar badge."}, 500 

        logger.log_debug(f"[business] Gerando Badge no banco.")
        success = db.insert_badge(badge_guid, badge_hash, owner_name, issuer_name, signed_hash, badge_base64)
        if not success:
            logger.log_error("Falha ao inserir o badge no banco de dados.")
        ...
        stack_trace = traceback.format_exc()
        logger.log_error(f"Erro ao gerar badge: {str(e)}\nStack Trace:\n{stack_trace}")
        return {"error": "Erro interno no servidor"}, 500
...
def badge_image(data):
    try:
        ...
        if 'badge_guid' not in data:
            logger.log_error("Dados de entrada faltando: 'badge_guid'")
            return {"error": "Dados de entrada inválidos"}, 400

        badge_guid = data['badge_guid']
        
        logger.log_debug(f"Recuperando badge para {badge_guid}.")
        ...
        if row:
            ...
            logger.log_error("Falha ao rechperar o badge no banco de dados.")
            return {"error": "Badge não encontrado"}, 404
    except Exception as e:
        stack_trace = traceback.format_exc()
        logger.log_error(f"Erro ao recuoerar badge: {str(e)}\nStack Trace:\n{stack_trace}")
        return {"error": "Erro interno no servidor"}, 500
...
def badge_valid(data):
    try:
        # Validação e análise dos dados recebidos
        if 'data' not in data:
            logger.log_error("Dados de entrada faltando: 'data'")
            return {"error": "Dados de entrada inválidos"}, 400

        encrypted_data = data['data']
                
        logger.log_debug("Analisando dados enviados.")

        if not encrypted_data:
            logger.log_error("Dados criptogtafados não informados.")
            return {"error": "Dados criptografados são obrigatórios"}, 400
            
        logger.log_debug(f"Descriptografando dados enviados: {encrypted_data}")

        decrypted_data = helpers.decrypt_data(encrypted_data)
        if not decrypted_data:
            logger.log_error("Não foi possível descriptograr dados informados.")
            return {"error": "Falha na descriptografia"}, 400 

        logger.log_debug(f"Decodificando dados enviados: {encrypted_data}")

        try:
            badge_guid, owner_name, issuer_name = decrypted_data.data.split("|")
            logger.log_debug(f"Validando badge {badge_guid} emitido por {issuer_name}")

        except ValueError:
            stack_trace = traceback.format_exc()
            logger.log_error("Não foi possível decodificar dados informados.")
        ...
        stack_trace = traceback.format_exc()
        logger.log_error(f"Erro ao validar badge: {str(e)}\nStack Trace:\n{stack_trace}")
        return {"error": "Erro interno no servidor"}, 500
...
def badge_list(data):
    try:
        ...
        if 'user_id' not in data:
            logger.log_error("Dados de entrada faltando: 'user_id'")
            return {"error": "Dados de entrada inválidos"}, 400
        ...
        stack_trace = traceback.format_exc()
        logger.log_error(f"Erro ao listar badges: {str(e)}\nStack Trace:\n{stack_trace}")
        return {"error": "Erro interno no servidor"}, 500
...
def badge_holder(data):
    try:
        ...
        if 'badge_name' not in data:
            logger.log_error("Dados de entrada faltando: 'badge_name'")
            return {"error": "Dados de entrada inválidos"}, 400
        ...
        stack_trace = traceback.format_exc()
        logger.log_error(f"Erro ao recuperar detentores do badge: {str(e)}\nStack Trace:\n{stack_trace}")
        return {"error": "Erro interno no servidor"}, 500
...
def linkedin_post(data):
    try:
        ...
        stack_trace = traceback.format_exc()
        logger.log_error(f"Erro ao recuperar a mensagem do post do LinkedIn: {str(e)}\nStack Trace:\n{stack_trace}")
        return {"error": "Erro interno no servidor"}, 500
